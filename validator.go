...
func newRuleOpts(kind reflect.Kind, rule string, opts []string) ruleOpts {
...
func (s *ruleDef) ruleOptions(rule string) []string {
	if s == nil {
		return nil
	}
...
func (s *ruleDef) findRules(rules []string, fallback string) string {
	if s == nil {
		return fallback
	}
	for _, l := range s.rules {
